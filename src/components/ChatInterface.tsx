...
import { saveRun, saveArtefact, logAudit, persistenceMode } from '../services/persistence';
...
export const ChatInterface: React.FC<{ impact?: 'formatief'|'summatief'|'beleid' }> = ({ impact = 'formatief' }) => {
  ...
  const triggerProactiveNudge = async () => {
      ...
      try {
        if (runId) {
          await saveArtefact({ run_id: runId, kind: 'proposal', provider: 'gemini', model: null, content: response.text });
          await logAudit({ event_type: 'ai_response_saved', run_id: runId, payload: { has_analysis: !!response.analysis, has_mechanical: !!response.mechanical } });
        }
      } catch (e) {
        // persistence failures should not break the learning UI
      }
  ...
  const handleSendMessage = async (textOverride?: string) => {
    ...
    const ssotVersion = core?.metadata?.version || core?.metadata?.system || 'unknown';
    const actorId = getOrCreateUserId();
    const runRow = {
      actor_type: 'leerling',
      actor_id: actorId,
      impact: impact as any,
      workflow_id: 'student_studio_chat',
      ssot_version: String(ssotVersion),
      status: (impact === 'summatief' ? 'waiting_human' : 'running') as any,
      input_ref: userMessage.text,
      notes: null,
    };
    const runSaved = await saveRun(runRow);
    const runId = (runSaved as any).data?.id || undefined;
    await logAudit({ event_type: 'student_message_sent', run_id: runId || null, payload: { mode: persistenceMode(), text: userMessage.text } });

    if (impact === 'summatief') {
      // Human gate: do not call the model yet
      try {
        if (runId) await logAudit({ event_type: 'human_gate_required', run_id: runId, payload: { impact } });
      } catch {}
      const gateMsg: Message = {
        id: Date.now().toString(),
        text: "Deze vraag is als 'summatief' gemarkeerd. Een docent moet dit eerst goedkeuren voordat AI begeleiding wordt gegeven.",
        sender: 'ai',
        timestamp: new Date(),
      };
    ...
    try {
      ...
      try { await logAudit({ event_type: 'student_message_failed', run_id: runId || null, payload: { error: String((error as any)?.message || error) } }); } catch {}
    } finally {
      setIsLoading(false);
      setLoadingStatus('');
      lastInteractionRef.current = Date.now(); 
    ...
    <div className={`flex h-[100dvh] w-full ${currentTheme.bg} text-slate-200 overflow-hidden font-sans transition-colors duration-[1500ms] relative`}>
      ...
      <div className={`flex-1 flex flex-col h-full relative z-0 transition-all duration-300 ${isDesktopDashboardOpen ? 'lg:mr-96' : ''}`}>
        ...
        <div className="h-14 flex items-center justify-between px-4 lg:hidden bg-slate-900/50 backdrop-blur-md z-20 shrink-0">
            <div className="font-bold text-teal-500" onClick={() => navigate('/')}>EAI STUDIO</div>
            ...
            <div className="max-w-3xl mx-auto min-h-full flex flex-col justify-end pb-4">
                ...
                {messages.length === 0 && !isBooting && !showProfileSetup && (
                    <div className="my-auto w-full flex flex-col items-center animate-in fade-in zoom-in duration-700">
                        ...
                        <p className="text-sm text-slate-500 mb-8 max-w-xs text-center">Het systeem is klaar. Start een sessie om de didactische engine te activeren.</p>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 w-full max-w-md">
                            <button onClick={() => setShowProfileSetup(true)} className="p-4 bg-teal-900/20 border border-teal-500/30 rounded-xl hover:bg-teal-900/40 transition-all text-left group">
                                <div className="text-teal-400 font-bold text-xs uppercase mb-1 group-hover:text-teal-300">Nieuwe Sessie</div>
                                <div className="text-slate-300 text-sm">Start volledige setup</div>
                            </button>
